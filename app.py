...
def submit_data():
    try:
        ...
        required_fields = [
            ...
            'no_of_model_changeover', 'model_changeover_time', 'per_hour_OT_cost', 'std_mp','ramp_up_efficiency',
            'ramp_up_days','ramp_up_line','working_hour_ramp_up','ramp_down_efficiency','ramp_down_days', 'working_hours_ramp_down', 'shift', 'working_hour_opr',
        ...
        ramp_up_eff = float(data['ramp_up_efficiency'])        
        ramp_up_days = int(data['ramp_up_days'])
        ramp_up_line = int(data['ramp_up_line'])
        working_hour_ramp_up= int(data['working_hour_ramp_up'])
        ramp_down_eff = float(data['ramp_down_efficiency'])
        ramp_down_days= int(data['ramp_down_days'])
        ...
        dl_ot_cost = round(per_hour_OT_cost * dl_ot_mh, 2)
        support_dl_ot_cost = round(ttl_support_dl * ot_hours_calc * per_hour_OT_cost * working_days, 2)
